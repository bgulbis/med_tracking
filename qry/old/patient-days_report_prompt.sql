WITH UNIT_ENCNTR_LIST AS (
	...
	FROM
		ENCNTR_LOC_HIST
	WHERE
		ENCNTR_LOC_HIST.LOC_NURSE_UNIT_CD IN DECODE(
	...
	WHERE
		...
		AND UNIT_TRNSFRS.LOC_NURSE_UNIT_CD IN DECODE(
			@Prompt('Nursing Units', 'A', {'HH CVICU', 'HH STRK', 'HH 7J'}, multi, free, persistent,, User:2),
			'HH CVICU', 5541,
			'HH STRK', 193270,
			'HH 7J', 283905037
		)
		AND UNIT_TRNSFRS.TRANSFER_DATE BETWEEN ADD_MONTHS(TRUNC(ADD_MONTHS(SYSDATE, 6), 'YEAR'), -@Prompt('Months Back', 'N', , mono, free, persistent, {'30'}, User:1)) AND TRUNC(SYSDATE, 'MONTH') - (1 / 86400)
	GROUP BY
		UNIT_TRNSFRS.TRANSFER_DATE,
		UNIT_TRNSFRS.LOC_NURSE_UNIT_CD
), CENSUS AS (
	...
	WHERE
		LH_CNT_LOC_UNIT_CENSUS.LOC_NURSE_UNIT_CD IN DECODE(
			@Prompt('Nursing Units', 'A', {'HH CVICU', 'HH STRK', 'HH 7J'}, multi, free, persistent,, User:2),
			'HH CVICU', 5541,
			'HH STRK', 193270,
			'HH 7J', 283905037
