...
WITH ALL_MEDS AS (
	...
	FROM
		(
			SELECT 
				ADD_MONTHS(TRUNC(ADD_MONTHS(SYSDATE, 6), 'YEAR'), -@Prompt('Months Back', 'N', , mono, free, persistent, {'30'}, User:1)) START_DATE,
	...
	FROM
		CODE_VALUE
	WHERE
		--CODE_VALUE.CODE_VALUE = 5541 -- HH CVICU
		--CODE_VALUE.CODE_VALUE = @Prompt('Nursing Units', 'N', , mono, free, persistent,, User:2)
		
		CODE_VALUE.CODE_VALUE IN DECODE(
			@Prompt('Nursing Units', 'A', {'HH CVICU', 'HH STRK', 'HH 7J'}, multi, free, persistent,, User:2),
			'HH CVICU', 5541,
			'HH STRK', 193270,
			'HH 7J', 283905037
		)
	...
	FROM
		...
		ENCOUNTER
	WHERE
		ENCNTR_LOC_HIST.END_EFFECTIVE_DT_TM >= pi_to_gmt(ADD_MONTHS(TRUNC(ADD_MONTHS(SYSDATE, 6), 'YEAR'), -@Prompt('Months Back', 'N', , mono, free, persistent, {'30'}, User:1)), pi_time_zone(2, @Variable('BOUSER')))
		AND ENCNTR_LOC_HIST.LOC_FACILITY_CD = 3310 -- HH HERMANN
		AND ENCNTR_LOC_HIST.LOC_BUILDING_CD = 216053283 -- HH HVI
		AND ENCNTR_LOC_HIST.LOC_NURSE_UNIT_CD IN DECODE(
			@Prompt('Nursing Units', 'A', {'HH CVICU', 'HH STRK', 'HH 7J'}, multi, free, persistent,, User:2),
			'HH CVICU', 5541,
			'HH STRK', 193270,
			'HH 7J', 283905037
	...
	WHERE
		...
		AND CLINICAL_EVENT.EVENT_END_DT_TM BETWEEN
			pi_to_gmt(ADD_MONTHS(TRUNC(ADD_MONTHS(SYSDATE, 6), 'YEAR'), -@Prompt('Months Back', 'N', , mono, free, persistent, {'30'}, User:1)), pi_time_zone(2, @Variable('BOUSER')))
	...
	WHERE
		...
		AND NURSE_UNIT_PTS.ENCNTR_LOC_HIST_ID = (
			...
			WHERE
				...
				AND NUP.LOC_NURSE_UNIT_CD IN DECODE(
					@Prompt('Nursing Units', 'A', {'HH CVICU', 'HH STRK', 'HH 7J'}, multi, free, persistent,, User:2),
					'HH CVICU', 5541,
					'HH STRK', 193270,
					'HH 7J', 283905037
