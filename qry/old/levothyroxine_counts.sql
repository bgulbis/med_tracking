...
FROM
	...
	ENCOUNTER
WHERE
	CLINICAL_EVENT.EVENT_CD = 37557425
	...
	AND (
		...
		AND CE_MED_RESULT.ADMIN_ROUTE_CD IN (508984, 9022513, 9022647, 9022649)
	)
	AND (
		CLINICAL_EVENT.EVENT_ID = CE_MED_RESULT.EVENT_ID
		AND (
		    CE_MED_RESULT.IV_EVENT_CD = 688706
		    OR (CE_MED_RESULT.ADMIN_DOSAGE > 0 AND CE_MED_RESULT.IV_EVENT_CD = 0)
		)
	)	 
	AND (
