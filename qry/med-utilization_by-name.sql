...
WHERE
    ...
    AND (
        ...
        AND CV_FACILITY.CODE_SET = 220
    )
	AND (
		CLINICAL_EVENT.ENCNTR_ID = ENCNTR_LOC_HIST.ENCNTR_ID
		AND CV_FACILITY.CODE_VALUE = ENCNTR_LOC_HIST.LOC_FACILITY_CD
	...
	AND (
		...
		AND (
		    ...
		    OR (
		        CV_IV_EVENT.DISPLAY = 'Begin Bag'
		        AND CV_IV_EVENT.CODE_SET = 180
		        AND CV_IV_EVENT.CODE_VALUE = CE_MED_RESULT.IV_EVENT_CD
		    )
	    )
	)
	AND (
	    ...
	    OR (
	        CV_ADMIN_ROUTE.DISPLAY IN ('INJ', 'IV', 'IVP', 'IVPB')
	        AND CV_ADMIN_ROUTE.CODE_SET = 4001
	        AND CV_ADMIN_ROUTE.CODE_VALUE = CE_MED_RESULT.ADMIN_ROUTE_CD
	    ) 
	)
	AND (
