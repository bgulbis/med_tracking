SELECT DISTINCT
	TO_CHAR(TRUNC(pi_from_gmt(ORDERS.ORIG_ORDER_DT_TM, (pi_time_zone(1, @Variable('BOUSER'))))), 'YYYY-MM-DD') AS EVENT_DATE,
	COUNT(ORDERS.ORDER_ID) AS DOSES
FROM
	ENCNTR_LOC_HIST,
	ENCOUNTER,
	ORDERS
...
GROUP BY
    TRUNC(pi_from_gmt(ORDERS.ORIG_ORDER_DT_TM, (pi_time_zone(1, @Variable('BOUSER')))))
