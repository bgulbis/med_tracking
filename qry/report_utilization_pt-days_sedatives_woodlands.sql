WITH START_MONTH AS (
	...
	WHERE
		...
		AND ENCNTR_LOC_HIST.LOC_FACILITY_CD = 9351127 -- TW The Woodland
	...
	FROM
		PATIENT_DAYS	
	WHERE
		pi_get_cv_display(PATIENT_DAYS.LOC_NURSE_UNIT_CD) <> 'TW PATW'
	GROUP BY
	...
	FROM 
		ALL_CENSUS
	WHERE
		FACILITY = 'TW The Woodland'
		AND NURSE_UNIT NOT IN ('TW PATW', 'TW NNTW')
	GROUP BY
		CENSUS_DT_TM,
		FACILITY
), FACILITY_ADMITS AS (
	...
	FROM
		...
		START_MONTH
	WHERE
		ENCOUNTER.ORGANIZATION_ID = 5022359 -- Memorial Hermann The Woodlands Hospital
		...
		AND ENCOUNTER.LOC_FACILITY_CD = 9351127 -- TW The Woodland
	GROUP BY
	...
	WHERE
		ENCOUNTER.DISCH_DT_TM BETWEEN
			pi_to_gmt(START_MONTH.START_MONTH, 'America/Chicago')
			AND pi_to_gmt(TRUNC(SYSDATE, 'DAY') - 1/86400, 'America/Chicago')
		AND ENCOUNTER.ORGANIZATION_ID = 5022359 -- Memorial Hermann The Woodlands Hospital
		AND ENCOUNTER.ENCNTR_TYPE_CLASS_CD IN (
			42631, -- Inpatient
			-- 55851, -- Emergency
			688523 -- Observation
