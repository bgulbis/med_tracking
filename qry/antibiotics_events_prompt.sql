SELECT DISTINCT
	...
	CV_EVENT_LOC.DISPLAY AS NURSE_UNIT_EVENT,
	...
	PHA_PRODUCT.GENERIC_NAME AS PRODUCT_NAME,
	PHA_PRODUCT.CDM AS CDM,
	PHA_PRODUCT.QTY AS QUANTITY
FROM
	CE_MED_RESULT,
	CLINICAL_EVENT,
	CODE_VALUE CV_ENCOUTER_TYPE,
	CODE_VALUE CV_EVENT_ADMIN,
	CODE_VALUE CV_EVENT_LOC,
	...
	PHA_PRODUCT
WHERE
	CLINICAL_EVENT.EVENT_CD IN (37556681, 117038716, 627676649, 1370510034, 1505894070)
	AND (
	...
	AND (
		...
		AND MED_ADMIN_EVENT.NURSE_UNIT_CD = CV_EVENT_LOC.CODE_VALUE(+)
	)
	AND (
		CASE ORDERS.TEMPLATE_ORDER_ID
			WHEN 0 THEN CLINICAL_EVENT.ORDER_ID
		...
		AND TEMPLATE_ORDERS.ORDER_ID = ORDER_PRODUCT.ORDER_ID(+)
		AND ORDER_PRODUCT.ITEM_ID = PHA_PRODUCT.ITEM_ID(+)
	)
	AND (
