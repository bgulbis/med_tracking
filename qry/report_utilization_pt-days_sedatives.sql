WITH START_MONTH AS (
	...
	WHERE
		...
		AND CLINICAL_EVENT.EVENT_END_DT_TM BETWEEN
			pi_to_gmt(START_MONTH.START_MONTH, 'CST')
		...
		CASE
			WHEN 
				LAG(ENCNTR_LOC_HIST.LOC_NURSE_UNIT_CD, 1, 0) OVER (
					PARTITION BY ENCNTR_LOC_HIST.ENCNTR_ID 
					ORDER BY ENCNTR_LOC_HIST.ENCNTR_LOC_HIST_ID
				) <> ENCNTR_LOC_HIST.LOC_NURSE_UNIT_CD THEN TRUNC(pi_from_gmt(ENCNTR_LOC_HIST.BEG_EFFECTIVE_DT_TM, 'CST'))
			WHEN
				LEAD(ENCNTR_LOC_HIST.LOC_NURSE_UNIT_CD, 1, 0) OVER (
					PARTITION BY ENCNTR_LOC_HIST.ENCNTR_ID 
					ORDER BY ENCNTR_LOC_HIST.ENCNTR_LOC_HIST_ID
				...
				AND LAG(ENCNTR_LOC_HIST.LOC_NURSE_UNIT_CD, 1, 0) OVER (
					PARTITION BY ENCNTR_LOC_HIST.ENCNTR_ID 
					ORDER BY ENCNTR_LOC_HIST.ENCNTR_LOC_HIST_ID
				) = ENCNTR_LOC_HIST.LOC_NURSE_UNIT_CD THEN TRUNC(pi_from_gmt(ENCNTR_LOC_HIST.END_EFFECTIVE_DT_TM, 'CST'))
	...
	WHERE
		...
		AND UNIT_TRNSFRS.TRANSFER_DATE BETWEEN pi_to_gmt(START_MONTH.START_MONTH, 'CST') AND pi_to_gmt(TRUNC(SYSDATE, 'DAY') - 1/86400, 'CST')
	GROUP BY
		UNIT_TRNSFRS.TRANSFER_DATE,
		UNIT_TRNSFRS.LOC_NURSE_UNIT_CD
), CENSUS AS (
	SELECT DISTINCT
		TRUNC(pi_from_gmt(LH_CNT_LOC_UNIT_CENSUS.CENSUS_DT_TM, 'CST')) AS CENSUS_DATE,
	...
	SELECT 
		TRUNC(pi_from_gmt(LH_CNT_LOC_UNIT_CENSUS.CENSUS_DT_TM, 'CST')) AS CENSUS_DT_TM,
	...
	SELECT
		TRUNC(pi_from_gmt(REG_DT_TM, 'CST')) AS REG_DT_TM,
	...
	GROUP BY
		TRUNC(pi_from_gmt(REG_DT_TM, 'CST'))
), FACILITY_DISCH AS (
	SELECT
		TRUNC(pi_from_gmt(DISCH_DT_TM, 'CST')) AS DISCH_DT_TM,
	...
	GROUP BY
		TRUNC(pi_from_gmt(DISCH_DT_TM, 'CST'))
), FACILITY_PT_DAYS AS (
	SELECT
		CENSUS_DT_TM,
		CENSUS,
