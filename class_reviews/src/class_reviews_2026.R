...
raw_df <- get_xlsx_data(paste0(f, "raw"), "mhhs_purchases_2025-03_2026-02", sheet = 1) |> 
    ...
    rename(
        ...
        units_shipped = `total shipped tabs, ml, units`
    )

zzz_prods <- distinct(raw_df, prod_desc) |> arrange(prod_desc)
...
write.xlsx(df_totals, paste0(f, "final/class_review_cost_data_2026.xlsx"), overwrite = TRUE)
