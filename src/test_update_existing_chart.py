...
prs = Presentation("../report/sidney/test.pptx")
slide = prs.slides[1]
chart = slide.shapes[0].chart

test_df = df_pvt.loc['Acetaminophen IV'].copy()
test_df.loc['2018-08-01', 'FY20'] = 1700

chart_data = CategoryChartData()
chart_data.categories = test_df.index

for j in range(0, len(test_df.columns)):
    if j == 3:
        ser = test_df.iloc[:, j]
        ...
        chart_data.add_series(test_df.columns[j], keep)
    else:
        chart_data.add_series(test_df.columns[j], test_df.iloc[:, j])
        
chart.replace_data(chart_data)
prs.save("../report/sidney/test_replace_existing.pptx")
